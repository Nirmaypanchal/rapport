...
- [ ] **Fix duplicate `needs-human` issues** — `needs-human.yml`'s dedupe check (`gh issue list --label needs-human --search "\"$title\" in:title"`) missed an already-open issue and recreated it: [#3](https://github.com/Nirmaypanchal/rapport/issues/3) and [#6](https://github.com/Nirmaypanchal/rapport/issues/6) are the same "Create the Reddit bot account…" request, three hours apart, both still open. The workflow reruns every 6 hours by cron, so an unresolved file will keep re-opening (and re-emailing the owner) indefinitely until this is fixed. _Why:_ found by Community 2026-09-10 while checking open issues; undermines the one channel meant to reach the owner. _Size:_ S.
  - Acceptance: an unresolved `needs-human` file never opens a second issue for the same file. Build's reading: match on the file path (write it into the issue body as a marker and search for that), not on the title text — `gh issue list --search` full-text matching is what failed. Close #6 as a duplicate of #3 by hand in the same run.
  - Careful: this edits `.github/workflows/needs-human.yml`, and the `sprint-merge` bot's token may not be allowed to merge a PR that touches a workflow file. If the merge is refused, say so in the log and escalate rather than working around it.
