...
/** Find the exact words (Search), or put a question to the local model and get the moments behind its answer (Ask). */
function FindTab() {
  ...
  return (
    <div className="h-full overflow-y-auto px-4 pb-10 pt-5 sm:px-8 sm:pt-7">
      <div className="mx-auto max-w-[760px]">
        <h1 className="font-display text-[26px] font-semibold tracking-[-0.02em] sm:text-[30px]">{tab === "ask" ? "Ask your library" : "Search"}</h1>
        <Tabs value={tab} onValueChange={(v) => setTab(String(v))} className="mt-3">
          <TabsList variant="line" className="justify-start">
            <TabsTrigger value="search">Search</TabsTrigger>
            <TabsTrigger value="ask">Ask</TabsTrigger>
          </TabsList>
          <TabsContent value="search" keepMounted><FindTab /></TabsContent>
          <TabsContent value="ask" keepMounted><AskPanel active={tab === "ask"} /></TabsContent>
        </Tabs>
      </div>
    </div>
  );
